...
export default function JuegoPage() {
  ...
  // joinRoom cuando está el socket y el room
  useEffect(() => {
    if (!socket || !room) return;
    socket.emit("joinRoom", { room }); // apunte del aula: joinRoom
  }, [socket, room]);

  // escuchar jugadores conectados y estado de sala
  useEffect(() => {
    if (!socket) return;
  ...
  // si hay menos de 2 jugadores mostramos "esperando"
  if (!ready) {
    return (
  ...
  // si está lista la sala, renderizamos el Juego
